...
if (root) then 
  ...
  !print*, 
  !print*, 'Please select the case you want to study by inputting the corrosponding integer:'
  !print*, ' 0 -  Ellipsoid' 
  !print*, ' 1 -  Uniform Magnetised Sphere'
  !print*, ' 2 -  Infinite plane'
  !print*, 
  ...
  open(file='geom_init.dat',unit=10,status='replace')
  ...
  !#################### ELLIPSOID TIIIMMMMMEEE ###################

  else if (analytic.eq.0) then
...
if (.not.root) then 
  ...
  open(file='geom_init.dat',unit=10,action='read')
...
!##############################################################
!-------------------------------------------------------------
!               Write arrays to file for each rank
!------------------------------------------------------------- 
!##############################################################

! to create an ID for each rank we will use the function id_str()
! this function takes an integer (in this case the rank) and converts 
! it to a string

rank_id = 20 + my_rank

fname = 'geom_test_rank'//trim(id_str(my_rank))//'.dat'
...
split(1) = floor(real(2*dim_x+1)/real(num_procs,kind=dp))

split(2) = floor(real(2*dim_y+1)/real(num_procs,kind=dp))

split(3) = floor(real(2*dim_z+1)/real(num_procs,kind=dp))
